...
{
	float inOut;
	float pSphere,kSphereReduction;
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < SphereDiameter)
			{
				...
				//VolumeOfSphere = (PI/24.0)*(2*SphereDiameter - d.w)(SphereDiameter+d.w)(SphereDiameter+d.w);
				...
				if(inOut < 0.0) magnitude = pSphere*(intersectionArea); // If inOut is negative the sphere are converging.
				else magnitude = kSphereReduction*pSphere*(intersectionArea); // If inOut is positive the sphere are diverging.
