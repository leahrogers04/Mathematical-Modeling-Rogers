	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < SphereDiameter)
			{
				...
				if(inOut < 0.0) magnitude = pSphere*(intersectionArea)/bodyVolume; // If inOut is negative the sphere are converging.
				else magnitude = kSphereReduction*pSphere*(intersectionArea)/bodyVolume; // If inOut is positive the sphere are diverging.
