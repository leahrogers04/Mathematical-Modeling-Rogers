...
void getForces()
{
	...
	for(int i = 0; i < NUMBER_OF_BALLS; i++)
	{
		...
		for(int j = i + 1; j < NUMBER_OF_BALLS; j++)
		{
			...
			{
				// scalling the vectors so that they have a length of one (making them unit vectors? yes i think)
				//uv stands for unit vector
				float uvx = dx / distance;
				float uvy = dy / distance;
				float uvz = dz / distance;

				//the relative velocity 
				//rv stand for relative velocity
				float rvx = Velocity[i].x - Velocity[j].x;
				float rvy = Velocity[i].y - Velocity[j].y;
				float rvz = Velocity[i].z - Velocity[j].z;

				//taking the cv product of the relative velocities and the unit vectors because it will give the velocity in the direction of the collision
				//cv stands for collison vector
				float cv = uvx * rvx + uvy * rvy + uvz * rvz;

				//making it so that when the balls collide they will change direction
