...
void setInitailConditions()
{
	...
	double density;
	double volume;
	double potentialWell = 0.165678; //pg*microns^2/littleSeconds^2
	//double depletionForce = 0.0;
	...
	diameter = 1.0; // micron
	density = 1.05; // picograms/microns^3
	volume = (PI/6.0)*diameter*diameter*diameter; // microns^3
	mass = density*volume; //pg
	
	printf("\n Mass of Polystyrene = %e kilograms", mass);
	printf("\n Diameter of Polystyrene = %e kilometers", diameter);
	printf("\n Density of Polystyrene = %e kilograms/kilometer^3", density);
	printf("\n Volume of Polystyrene = %e kilometers^3", volume);
	printf("\n");
	...
	for(int i = 0; i < NUMBER_OF_BODIES; i++)
	{
		...
		BodyRadius[i] = diameter/2.0;
	}
	
	
	MassUnitConverter = 1.0; // pg
	LengthUnitConverter = diameter; // mg
	TimeUnitConverter =10e-4; // 1 little second
	...
	depletionForce = potentialWell/0.08;//pg*microns/littleSeconds^2
	printf("\n The magnitude of the depletionForce = %f in our units", depletionForce);
	...
	MaxVelocity = 0.5;
	printf("\n Max velocity = %f um/littleSeconds", MaxVelocity*LengthUnitConverter/TimeUnitConverter);
...
void getForces()
{
	...
	for(int i = 0; i < NUMBER_OF_BODIES; i++)
		...
		// This adds forces between polystyrene spheres
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < (BodyRadius[i] + BodyRadius[j]))
			{
				...
				float r2 = fmin(BodyRadius[j], BodyRadius[i]);
				...
				if(inOut < 0.0) depletionForce = kSphere*intersectionArea; // If inOut is negative the sphere are converging.
				else depletionForce = kSphereReduction*kSphere*intersectionArea; // If inOut is positive the sphere are diverging.
			...
			if(d.w <= 1.08)
			{
				// This adds the depletion force between the spheres
