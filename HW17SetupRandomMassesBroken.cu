...
void setInitailConditions()
{
	...
	for (int i = 0; i < NUMBER_OF_BODIES; i++)
	{
		...
		printf("\n Body %d has a mass of %f kilograms with a radius of %f kilometers", i, BodyMass[i], BodyRadius[i]);
	}
	...
	for(int i = 0; i < NUMBER_OF_BODIES; i++)
	{
		...
		Color[i].x = (float)rand() / RAND_MAX;
		Color[i].y = (float)rand() / RAND_MAX;
		Color[i].z = (float)rand() / RAND_MAX;
...
void getForces()
{
	...
	float minSeparation;
	//float sphereRadius = BodyRadius[i]/2.0;
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < BodyRadius[i]+BodyRadius[j])
			{
				...
				if(d.w > minSeparation)
				{
					float rad1 = BodyRadius[i];
					float rad2 = BodyRadius[j];
					float diameter = rad1 + rad2;
				//float minimumSeparation = (BodyRadius[i]+BodyRadius[j])*(0.01);
				float repulsion = kSphereReduction*diameter;
				
				intersectionArea = PI * ((rad2*rad2) - pow(((rad1*rad1 - rad2*rad2 - d.w*d.w)/(-2*d.w)),2));
				//PI * (rad2*rad2 -(pow(((rad1*rad1 - rad2*rad2 - diameter*diameter)/(-2*diameter)), 2)));
				...
				//if(d.w < repulsion)
				//{
				//	magnitude = kSphere*(1.0 - d.w/repulsion);
				//}
